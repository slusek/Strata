...
public class DiscountingSwapLegPricerTest {
  ...
  public void test_annuityCash_onePeriod() {
    double forward = 0.01;
    DiscountingSwapLegPricer test = DiscountingSwapLegPricer.DEFAULT;
    double computed = test.annuityCash(FIXED_EXPANDED_SWAP_LEG_REC_USD, forward);
    double expected = SwapDummyData.NOTIONAL * (1d - 1d / (1d + forward / 4d)) / forward;
  ...
  public void test_annuityCash_twoPeriods() {
    ...
    double forward = 0.01;
    DiscountingSwapLegPricer test = DiscountingSwapLegPricer.DEFAULT;
    double computed = test.annuityCash(leg, forward);
    double expected = SwapDummyData.NOTIONAL * (1d - Math.pow(1d + forward / 4d, -2)) / forward;
    assertEquals(computed, expected, SwapDummyData.NOTIONAL * TOLERANCE);
  }
  ...
  public void test_annuityCashDerivative_onePeriod() {
    double forward = 0.01;
    DiscountingSwapLegPricer test = DiscountingSwapLegPricer.DEFAULT;
    double computed = test.annuityCashDerivative(FIXED_EXPANDED_SWAP_LEG_REC_USD, forward);
    double expected = 0.5 * (test.annuityCash(FIXED_EXPANDED_SWAP_LEG_REC_USD, forward + FD_SHIFT)
        - test.annuityCash(FIXED_EXPANDED_SWAP_LEG_REC_USD, forward - FD_SHIFT)) / FD_SHIFT;
  ...
  public void test_annuityCashDerivative_twoPeriods() {
    ...
    double forward = 0.01;
    DiscountingSwapLegPricer test = DiscountingSwapLegPricer.DEFAULT;
    double computed = test.annuityCashDerivative(leg, forward);
    double expected = 0.5 / FD_SHIFT
        * (test.annuityCash(leg, forward + FD_SHIFT) - test.annuityCash(leg, forward - FD_SHIFT));
    assertEquals(computed, expected, SwapDummyData.NOTIONAL * FD_SHIFT);
  }
