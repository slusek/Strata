...
public class BlackBondFutureOptionMarginedProductPricerTest {
  ...
  private static final ZoneId ZONE = FUTURE_OPTION_PRODUCT.getExpirationZone();
  ...
  public void test_price() {
    ...
    double vol = SURFACE.zValue(expirationTime, logMoneyness);
    double expected = BlackFormulaRepository.price(futurePrice, strike, expirationTime, vol, true);
  ...
  public void test_delta() {
    ...
    double logMoneyness = Math.log(strike / futurePrice);
    double vol = SURFACE.zValue(expirationTime, logMoneyness);
    double expected = BlackFormulaRepository.delta(futurePrice, strike, expirationTime, vol, true);
    assertEquals(computed, expected, TOL);
  }
  ...
  public void test_gamma() {
    ...
    double logMoneyness = Math.log(strike / futurePrice);
    double vol = SURFACE.zValue(expirationTime, logMoneyness);
    double expected = BlackFormulaRepository.gamma(futurePrice, strike, expirationTime, vol);
    assertEquals(computed, expected, TOL);
  }
  ...
  public void test_theta() {
    ...
    double logMoneyness = Math.log(strike / futurePrice);
    double vol = SURFACE.zValue(expirationTime, logMoneyness);
    double expected = BlackFormulaRepository.driftlessTheta(futurePrice, strike, expirationTime, vol);
    assertEquals(computed, expected, TOL);
  }
  ...
  public void test_priceSensitivity() {
    ...
    double vol = SURFACE.zValue(expirationTime, logMoneyness);
    double volUp = SURFACE.zValue(expirationTime, logMoneynessUp);
    double volDw = SURFACE.zValue(expirationTime, logMoneynessDw);
    double volSensi = 0.5 * (volUp - volDw) / EPS;
    double vega = BlackFormulaRepository.vega(futurePrice, strike, expirationTime, vol);
    CurveCurrencyParameterSensitivities sensiVol = RATE_PROVIDER.curveParameterSensitivity(
        FUTURE_PRICER.priceSensitivity(FUTURE_OPTION_PRODUCT.getUnderlying(), RATE_PROVIDER))
  ...
  private void testPriceSensitivityBlackVolatility(
    ...
    for (int i = 0; i < nVol; ++i) {
      ...
      assertEquals(computed.getSensitivity()[index], expected, EPS);
    }
  }
