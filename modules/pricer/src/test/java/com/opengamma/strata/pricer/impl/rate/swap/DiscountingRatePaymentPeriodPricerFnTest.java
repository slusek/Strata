...
public class DiscountingRatePaymentPeriodPricerFnTest {
  ...
  public void test_presentValue_single() {
    ...
    when(prov.getValuationDate()).thenReturn(VALUATION_DATE);
    when(prov.discountFactor(USD, PAYMENT_DATE_1)).thenReturn(DISCOUNT_FACTOR);
  ...
  public void test_futureValue_single_fx() {
    RatesProvider prov = mock(RatesProvider.class);
    when(prov.getValuationDate()).thenReturn(VALUATION_DATE);
    when(prov.fxIndexRate(WM_GBP_USD, GBP, FX_DATE_1)).thenReturn(RATE_FX);
  ...
  public void test_futureValue_compoundNone() {
    ...
    double fvComputed = DiscountingRatePaymentPeriodPricer.DEFAULT.futureValue(PAYMENT_PERIOD_FULL_GS, prov);
    assertEquals(fvComputed, fvExpected, TOLERANCE_PV);
  }
  ...
  public void test_futureValue_compoundNone_fx() {
    ...
    double fvComputed = DiscountingRatePaymentPeriodPricer.DEFAULT.futureValue(PAYMENT_PERIOD_FULL_GS_FX, prov);
    assertEquals(fvComputed, fvExpected, TOLERANCE_PV);
  }
  ...
  public void test_futureValue_compoundFlat_notional() {
    ...
    RatesProvider prov = mock(RatesProvider.class);
    when(prov.getValuationDate()).thenReturn(VALUATION_DATE);
    double fvComputedNot = DiscountingRatePaymentPeriodPricer.DEFAULT.futureValue(periodNot, prov);
    double fvComputed1 = DiscountingRatePaymentPeriodPricer.DEFAULT.futureValue(period1, prov);
  ...
  */
  public void test_presentValueSensitivity_ibor_noCompounding() {
    ...
    when(prov.getValuationDate()).thenReturn(VALUATION_DATE);
    when(prov.relativeTime(PAYMENT_PERIOD_FLOATING.getPaymentDate())).thenReturn(paymentTime);
    when(prov.discountFactor(PAYMENT_PERIOD_FLOATING.getCurrency(), PAYMENT_PERIOD_FLOATING.getPaymentDate()))
        .thenReturn(DISCOUNT_FACTOR);
    PointSensitivityBuilder builder = ZeroRateSensitivity.of(PAYMENT_PERIOD_FLOATING.getCurrency(),
        PAYMENT_PERIOD_FLOATING.getPaymentDate(), -DISCOUNT_FACTOR * paymentTime); // this is implemented in provironment
    when(prov.discountFactorZeroRateSensitivity(PAYMENT_PERIOD_FLOATING.getCurrency(),
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      when(obsFunc.rateSensitivity(observation, dates[i], dates[i + 1], prov)).thenReturn(iborSense);
      when(obsFunc.rate(observation, dates[i], dates[i + 1], prov)).thenReturn(rates[i]);
    }
    PointSensitivities senseComputed = pricer.presentValueSensitivity(PAYMENT_PERIOD_FLOATING, prov).build();

    double eps = 1.e-7;
    List<IborRateSensitivity> senseExpectedList = futureFwdSensitivityFD(prov, PAYMENT_PERIOD_FLOATING, obsFunc, eps);
    PointSensitivities senseExpected = PointSensitivities.of(senseExpectedList).multipliedBy(DISCOUNT_FACTOR);
    List<ZeroRateSensitivity> dscExpectedList = dscSensitivityFD(prov, PAYMENT_PERIOD_FLOATING, obsFunc, eps);
  ...
  public void test_futureValueSensitivity_ibor_noCompounding() {
    ...
    PointSensitivities senseComputed = pricer.futureValueSensitivity(PAYMENT_PERIOD_FLOATING, prov).build();

    double eps = 1.e-7;
    List<IborRateSensitivity> senseExpectedList = futureFwdSensitivityFD(prov, PAYMENT_PERIOD_FLOATING, obsFunc,
        eps);
  ...
  private List<IborRateSensitivity> futureFwdSensitivityFD(RatesProvider prov, RatePaymentPeriod payment,
      RateObservationFn<RateObservation> obsFunc, double eps) {
    LocalDate valuationDate = prov.getValuationDate();
    ...
    for (int j = 0; j < nPeriods; ++j) {
      ...
      for (int i = 0; i < nPeriods; ++i) {
        ...
        double rate = obsFunc.rate(observation, period.getStartDate(), period.getEndDate(), prov);
        if (i == j) {
  ...
  private List<ZeroRateSensitivity> dscSensitivityFD(RatesProvider prov, RatePaymentPeriod payment,
      RateObservationFn<RateObservation> obsFunc, double eps) {
    LocalDate valuationDate = prov.getValuationDate();
    LocalDate paymentDate = payment.getPaymentDate();
    double discountFactor = prov.discountFactor(payment.getCurrency(), paymentDate);
    ...
    for (int i = 0; i < periods.size(); ++i) {
      ...
      double rate = obsFunc.rate(observation, startDate, endDate, prov);
