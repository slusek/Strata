 ...
 */
package com.opengamma.platform.pricer.results;
    ...
    if(nbZr != 0) {
      ...
      while(loopzr < nbZr) {
        ...
        while((loopzr < nbZr) &&
          ...
          double time = environment.relativeTime(valuationDate, listZR.get(loopzr).getDate());
          list.add(DoublesPair.of(time, listZR.get(loopzr).getValue()));
          loopzr++;
        }
        String currentName = environment.getMulticurve().getName(Currency.of(ccyDiscount.toString()));
        Pair<String, Currency> pair = Pairs.of(currentName, Currency.of(ccySensitivity.toString()));
        double[] sensiParam = environment.getMulticurve().parameterSensitivity(currentName, list);
    ...
    if(nbFr != 0) {
      ...
      while(loopfr < nbFr) {
        ...
        while((loopfr < nbFr) &&
          ...
          double fixingStartTime = environment.relativeTime(valuationDate, fixingStartDate);
          double fixingEndTime = environment.relativeTime(valuationDate, fixingEndDate);
        ...
        String currentName = environment.getMulticurve().getName(environment.convert(currentIndex));
        Pair<String, Currency> pair = Pairs.of(currentName, Currency.of(currentCcy.toString()));
        double[] sensiParam = environment.getMulticurve().parameterForwardSensitivity(currentName, list);
        result = result.plus(pair, new DoubleMatrix1D(sensiParam));
      }
    }
    return result;
