...
public final class BlackVolatilitySmileFxProvider implements BlackVolatilityFxProvider, ImmutableBean {
  ...
  public SurfaceCurrencyParameterSensitivity surfaceParameterSensitivity(FxOptionSensitivity point) {
    ...
    for (int i = 0; i < nTimes; ++i) {
      ...
      for (int j = 0; j < nDeltas; ++j) {
        deltasTotal[j] = 1d - deltas[j];
        deltasTotal[2 * nDeltas - j] = deltas[j];
      }
      for (int j = 0; j < nDeltasTotal; ++j) {
        sensiList.add(bucketedSensi[i][j] * pointValue);
        Delta absoluteDelta = new Delta(deltasTotal[j]);
    ...
    double[] sensi = ArrayUtils.toPrimitive((Double[]) sensiList.toArray());
    DefaultSurfaceMetadata metadata = DefaultSurfaceMetadata.builder()
        ...
        .build();
    SurfaceCurrencyParameterSensitivity sensitivity =
        SurfaceCurrencyParameterSensitivity.of(metadata, point.getCurrency(), sensi);
    return sensitivity;
  }
