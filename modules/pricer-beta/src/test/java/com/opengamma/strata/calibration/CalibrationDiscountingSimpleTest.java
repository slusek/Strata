...
public class CalibrationDiscountingSimpleTest {
  ...
  private static final CurveParameterMetadata[] FWD3_PARAM_METADATA = new CurveParameterMetadata[FWD3_NB_NODES];
...
//          FixedIborSwapTemplate.of(Period.ZERO, Tenor.of(FWD3_IRS_TENORS[i - 1]), USD_FIXED_6M_LIBOR_3M),
//          QuoteKey.of(StandardId.of(SCHEME, FWD3_ID_VALUE[i])));
//    }
    for (int i = 0; i < FWD3_NB_NODES; i++) {
      FWD3_PARAM_METADATA[i] = FWD3_NODES[i].metadata(VALUATION_DATE);
      FWD3_TIMES[i] = CURVE_DC.yearFraction(VALUATION_DATE, ((TenorCurveNodeMetadata) FWD3_PARAM_METADATA[i]).getDate());
      FWD3_TRADES.add(FWD3_NODES[i].trade(VALUATION_DATE, FWD3_QUOTES));
      FWD3_GUESS.add(0.0d);
    }
  }
  private static final CurveMetadata FWD3_CURVE_METADATA = 
      DefaultCurveMetadata.builder().curveName(FWD3_CURVE_NAME).xValueType(ValueType.YEAR_FRACTION)
  ...
  private static final InterpolatedCurveTemplate FWD3_TEMPLATE = new InterpolatedCurveTemplate(
      FWD3_CURVE_METADATA, FWD3_TIMES, EXTRAPOLATOR_FLAT, INTERPOLATOR_LINEAR, EXTRAPOLATOR_FLAT);
  ...
  public void calibration() {
    ...
    Pair<ImmutableRatesProvider, CurveBuildingBlockBundle> result =
        function.calibrate(dataTotal, knownData, DSC_NAMES, IDX_NAMES);
    // Test PV
    // Fixing 
    CurrencyAmount pvFixing = FIXING_PRICER.presentValue(((IborFixingDepositTrade)FWD3_TRADES.get(0)).getProduct(), result.getFirst());
    assertEquals(pvFixing.getAmount(), 0.0, TOLERANCE_PV);
    // FRA
    for (int i = 0; i < FWD3_NB_FRA_NODES; i++) {
      CurrencyAmount pvFra = FRA_PRICER.presentValue(((FraTrade)FWD3_TRADES.get(i+1)), result.getFirst());
