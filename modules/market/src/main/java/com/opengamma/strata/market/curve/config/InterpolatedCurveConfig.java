...
public final class InterpolatedCurveConfig
  ...
  @Override
  public Curve createCurve(LocalDate valuationDate, double[] parameters) {
    CurveMetadata meta = metadata(valuationDate);
    double[] nodeTimes = new double[getParameterCount()];
    for (int i = 0; i < getParameterCount(); i++) {
      nodeTimes[i] = getDayCount().get()
          .yearFraction(valuationDate, ((TenorCurveNodeMetadata) meta.getParameterMetadata().get().get(i)).getDate());
    }
    return InterpolatedNodalCurve.builder()
        .metadata(meta)
        .xValues(nodeTimes)
        .yValues(parameters)
        .extrapolatorLeft(leftExtrapolator)
        .interpolator(interpolator)
        .extrapolatorRight(rightExtrapolator).build();
  }
